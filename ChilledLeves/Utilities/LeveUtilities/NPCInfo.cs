...
namespace ChilledLeves.Utilities.LeveUtilities
{
    ...
    {
        private static HashSet<uint> IshgardTurnin = new() { 153, 151, 154, 156, 155, 152, 157 };
        private static HashSet<uint> KuganeTurnin = new() { 159, 161, 162, 171, 172, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194 };
        private static HashSet<uint> CrystariumTurnin = new() { 195, 196, 199, 200, 205, 206, 207, 208, 209, 210, 211, 212, 213, 214, 215, 216, 217, 218, 219, 220, 221, 222, 223, 224, 225, 226, 227, 228 };
        private static HashSet<uint> SharlayanTurnin = new() { 229, 230, 231, 232, 233, 234, 235, 236, 237, 238, 239, 240, 241, 242, 243, 244, 245, 246, 247, 248, 254, 255, 256 };
        private static HashSet<uint> TuliyoliTurnin = new() { 257, 258, 259, 260, 261, 262, 263, 264, 265, 266, 267, 268, 269, 270, 271, 272, 273, 274, 275, 276, 277, 278, 279, 280, 281, 282, };

        private static readonly Dictionary<uint, uint> LeveClientToNpcId = new()
        {
        ...
        public static uint TurninNpcId(uint leveClient, uint JobType = 0)
        {
            ...
            if (LeveClientToNpcId.TryGetValue(leveClient, out uint npcId))
                return npcId;

            // Handle special job-type cases
            if (IshgardTurnin.Contains(leveClient))
                return JobType == 18 ? 1011210u : 1011209u; // Cesteline : Fionnuala

            if (KuganeTurnin.Contains(leveClient))
                return JobType == 18 ? 1018999u : 1018998u; // Geimrael : Chantine

            if (CrystariumTurnin.Contains(leveClient))
                return JobType == 18 ? 1027849u : 1027848u; // Shue-Hann : Moyce

            if (SharlayanTurnin.Contains(leveClient))
                return JobType == 18 ? 1037265u : 1037264u; // Wurtwyb : Ahldiyrn

            if (TuliyoliTurnin.Contains(leveClient))
                return JobType == 18 ? 1048392u : 1048391u;
        ...
        public class LeveInfoVendor
        {
            ...
            public int BattlecraftButton { get; set; } = 0;
            public int GatheringButton { get; set; } = 0;
            public int CrafterButton { get; set; } = 0;
            public int LeaveButton { get; set; } = 0;
            public int LSGatherButton { get; set; } = 0;
            public int LSCrafterButton { get; set; } = 0;
