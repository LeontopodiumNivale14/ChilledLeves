...
namespace ChilledLeves.Ui.MainWindow_Tabs
{
    internal class Gathering_Profiles
    {
        public static void Draw()
        {
            ...
            {
                float sidebarWidth = 300 * ImGuiHelpers.GlobalScale;
                using (var leftChild = ImRaii.Child("Gather Buff: Profile Selection", new(sidebarWidth, ImGui.GetContentRegionAvail().Y), true))
                {
            ...
            ImGui.SameLine();
            using (var child_Profile = ImRaii.Child("Gather Buff: Profile Editor", default, false))
            {
                ProfileEditor();
            }
        ...
        private static unsafe void ProfileEditor()
        {
            ...
            if (C.FindGatherProfile(SelectedProfile) is { } gatherProfile)
            {
                ...
                float rightColumnWidth = rightColumnAvail.X;
                ...
                float headerHeight = ImGui.GetFrameHeightWithSpacing() * 4.4f;
                using (var headerChild = ImRaii.Child("Gather Buff: Profile Header", new(rightColumnWidth, headerHeight), true))
                {
                    if (headerChild.Success)
                        DrawProfileHeader(gatherProfile);
                }

                // Bottom row: buff priority list + buff details
                ...
                float buffListWidth = maxButtonWidth + 50;

                using (var buffChild = ImRaii.Child("Buff Selection Child", new(buffListWidth, remaining.Y), true))
                {
                    if (buffChild.Success)
                        DrawBuffPriorityList(gatherProfile);
                }

                ImGui.SameLine();

                float detailsWidth = remaining.X - buffListWidth - ImGui.GetStyle().ItemSpacing.X;
                using (var buffDetails = ImRaii.Child("Buff Details Selection", new(detailsWidth, remaining.Y), true))
                {
                    if (buffDetails.Success)
                        DrawBuffDetails(gatherProfile);
                }
            }
            else
            {
        ...
        private static void DrawProfileHeader(GatherProfile gatherProfile)
        {
            if (EditName)
            {
        ...
        private static unsafe void DrawBuffPriorityList(GatherProfile gatherProfile)
        {
        ...
        private static void DrawBuffDetails(GatherProfile gatherProfile)
        {
